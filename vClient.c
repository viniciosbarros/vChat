...
#define BUFSIZE 2048
#define WORD 32
#define SERVER_IP "192.168.201.160"	//Debian Parks Mirror
#define PORT 9055
...
int auth(struct User user)
{
	char pass[WORD];

	printf("%s ParksChat(%s) password: ", user.name, SERVER_IP);
	system("stty -echo");
	scanf("%s",&pass);
	system("stty echo");
...
int cmd_prompt(struct User user)
{
...
void close_session(struct User user, int sockfd)
{
...
void send_recv(int i, int sockfd, struct User user)
{
	char send_buf[BUFSIZE];
...
int connect_request(int *sockfd, struct sockaddr_in *server_addr)
{
