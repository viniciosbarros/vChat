...
void send_recv(int i, fd_set * master, int sockfd, int fdmax)
{
  ...
  /* 
     if ((nbytes_recvd = recv(i, recv_buf, BUFSIZE, 0)) <= 0) { if
     (nbytes_recvd == 0) { syslog(LOG_NOTICE,"socket %d terminated\n", i);
     }else { perror("recv"); } close(i); FD_CLR(i, master); }else {
     //printf("%s\n", recv_buf); for(j = 0; j <= fdmax; j++){ send_to_all(j, 
     i, sockfd, nbytes_recvd, recv_buf, master ); } } */
}
          ...
          struct sockaddr_in *client_addr)
{
  ...
  if ((newsockfd =
     accept(sockfd, (struct sockaddr *)client_addr, &addrlen)) == -1)
  {
    perror("accept");
    exit(1);
  }
